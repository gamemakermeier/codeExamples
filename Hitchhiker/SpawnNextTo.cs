...
{
	//this script is used to spawn prefabs relative to a target type of gameobjects to make mass placement/replacement easier for the leveldesign pipeline (e.g. spawn post boxes infront of every house or replace all post boxes with dog houses)
	[System.Serializable]
	...
	public class SpawnNextTo : MonoBehaviour
	{
		...
		public void Spawn()
		{
			...
			foreach (Transform child in possibleSpawnPoints)
			{
				...
				if (child.name == targetObjectRep.name || (targetAllInRange && child.transform != spawnScanRangeParent))
				{
